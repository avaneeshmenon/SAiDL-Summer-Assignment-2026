...
def run_hybrid_experiments():
    ...
    for conv_type in CONV_TYPES:
        ...
        final["context_length"] = CTX
        final["params"]         = model.count_parameters()
        final["epoch_time_avg"] = sum(history["epoch_time"]) / max(len(history["epoch_time"]), 1)
        final["peak_mem_mb"]    = max(history["peak_mem_mb"]) if history["peak_mem_mb"] else 0.0
...
def _plot_ppl_vs_compute(metrics_list, attn, pos, ctx):
    ...
    fig.suptitle(f"Hybrid designs | attn={attn} | pos={pos} | ctx={ctx}", fontsize=11)

    ax1.scatter(params, ppls, s=80, zorder=3)
    for label, x, y in zip(labels, params, ppls):
        ax1.annotate(label, (x, y), textcoords="offset points", xytext=(5, 3), fontsize=8)
    ...
    for label, x, y in zip(labels, thrpts, ppls):
        ax2.annotate(label, (x, y), textcoords="offset points", xytext=(5, 3), fontsize=8)
...
if __name__ == "__main__":
    ...
    if RUN_ATTENTION:
        ...
        print("Nothing to run. Set one of RUN_ATTENTION / RUN_POSITIONAL / RUN_HYBRID = True")